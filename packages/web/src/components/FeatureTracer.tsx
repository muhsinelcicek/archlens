...
export function FeatureTracer({ model, graphRef, className = "" }: FeatureTracerProps) {
  ...
  if (processes.length === 0) {
    return (
      <div className={`text-center text-[#5a5a70] text-xs py-4 ${className}`}>
  ...
  return (
    <div className={`${className}`}>
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 bg-elevated border border-zinc-700 rounded-lg px-3 py-1.5 text-xs text-[#8888a0] hover:bg-zinc-700 transition-colors min-w-[200px]"
          >
            ...
            <div className="absolute bottom-full left-0 mb-1 w-72 bg-surface border border-zinc-700 rounded-lg shadow-xl z-50 max-h-60 overflow-y-auto">
              {processes.filter((p) => p.category !== "presentation" && p.category !== "api-service").map((p) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-xs hover:bg-elevated transition-colors ${selectedProcess === p.id ? "bg-elevated text-archlens-400" : "text-[#8888a0]"}`}
                >
                  <div className="font-medium">{p.name}</div>
                  <div className="text-[#5a5a70] mt-0.5">{p.steps.length} steps — {p.category}</div>
                </button>
              ))}
            </div>
          )}
        ...
        {process && (
          <>
            <div className="flex items-center gap-1">
              <button onClick={() => goToStep(currentStep - 1)} className="p-1 rounded hover:bg-elevated text-[#5a5a70] hover:text-[#8888a0]" disabled={currentStep === 0}>
              ...
              <button onClick={() => goToStep(currentStep + 1)} className="p-1 rounded hover:bg-elevated text-[#5a5a70] hover:text-[#8888a0]" disabled={currentStep >= (process?.steps.length || 1) - 1}>
            ...
            <div className="flex items-center gap-0.5 flex-1 overflow-x-auto">
              {process.steps.map((step, i) => (
                <div key={i} className="flex items-center gap-0.5 flex-shrink-0">
                  {i > 0 && <ArrowRight className="h-2.5 w-2.5 text-[#5a5a70]" />}
                  <button
                    onClick={() => goToStep(i)}
                    className={`px-2 py-0.5 rounded text-[9px] font-medium whitespace-nowrap transition-colors ${
                      i === currentStep
                        ...
                        : i < currentStep
                          ? "bg-elevated text-[#8888a0]"
                          : "text-[#5a5a70] hover:text-[#8888a0]"
                    }`}
                  >
                    {step.name}
                  </button>
      ...
      {process && process.steps[currentStep] && (
        <div className="mt-2 rounded-lg bg-elevated border border-[#2a2a3a] px-3 py-2">
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-xs font-medium text-[#e4e4ed]">{process.steps[currentStep].name}</div>
              <div className="text-[10px] text-[#5a5a70] mt-0.5">{process.steps[currentStep].description}</div>
              <div className="flex items-center gap-2 mt-1 text-[9px] font-mono">
                ...
                <ArrowRight className="h-2.5 w-2.5 text-[#5a5a70]" />
