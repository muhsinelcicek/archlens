    ...
    <div className={`${className}`}>
      <div className="overflow-auto">
        <table className="border-collapse text-[10px]">
          <thead>
            <tr>
              <th className="p-1 text-zinc-600 text-right w-24">from \ to</th>
              {moduleNames.map((name) => (
                <th key={name} className="p-1 w-20">
                  <div className="flex flex-col items-center gap-0.5">
                    <div className="w-2 h-2 rounded-full" style={{ backgroundColor: getLayerColor(name) }} />
                    <span className="text-zinc-400 font-mono truncate max-w-[70px]" title={name}>{name}</span>
                    <span className="text-zinc-600">Ca:{metrics.get(name)?.ca || 0}</span>
                  </div>
                </th>
              ))}
              <th className="p-1 text-zinc-600 w-12">I</th>
            </tr>
          </thead>
          <tbody>
            {moduleNames.map((row) => {
                ...
                <tr key={row}>
                  <td className="p-1 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <span className="text-zinc-400 font-mono truncate max-w-[70px]" title={row}>{row}</span>
                  ...
                  {moduleNames.map((col) => {
                    ...
                    return (
                      <td
                        key={col}
                        className={`p-0 w-20 h-10 text-center relative cursor-pointer transition-all ${isSelf ? "bg-zinc-900" : ""}`}
                        style={{
                      ...
                      >
                        ...
                        {isSelf && <span className="text-zinc-700">-</span>}

                        {/* Tooltip */}
                        {isHovered && cell && cell.count > 0 && (
                          <div className="absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 shadow-xl whitespace-nowrap text-left">
                            <div className="text-xs font-bold text-zinc-200">{row} → {col}</div>
                            <div className="text-[10px] text-zinc-400 mt-1">{cell.count} dependencies</div>
                            {Object.entries(cell.types).map(([type, count]) => (
                              <div key={type} className="text-[10px] text-zinc-500">{type}: {count}</div>
                            ))}
