    ...
    <div className={`${className}`}>
      <div className="overflow-auto">
        <table className="border-collapse text-[10px]">
          <thead>
            <tr>
              <th className="p-1 text-[var(--color-text-muted)] text-right w-24">from \ to</th>
              {moduleNames.map((name) => (
                <th key={name} className="p-1 w-20">
                  <div className="flex flex-col items-center gap-0.5">
                    <div className="w-2 h-2 rounded-full" style={{ backgroundColor: getLayerColor(name) }} />
                    <span className="text-[var(--color-text-secondary)] font-mono truncate max-w-[70px]" title={name}>{name}</span>
                    <span className="text-[var(--color-text-muted)]">Ca:{metrics.get(name)?.ca || 0}</span>
                  </div>
                </th>
              ))}
              <th className="p-1 text-[var(--color-text-muted)] w-12">I</th>
            </tr>
          </thead>
          <tbody>
            {moduleNames.map((row) => {
                ...
                <tr key={row}>
                  <td className="p-1 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <span className="text-[var(--color-text-secondary)] font-mono truncate max-w-[70px]" title={row}>{row}</span>
                  ...
                  {moduleNames.map((col) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isSelf && <span className="text-[var(--color-text-muted)]">-</span>}
                        ...
                        {isHovered && cell && cell.count > 0 && (
                          <div className="absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 bg-surface border border-zinc-700 rounded-lg px-3 py-2 shadow-xl whitespace-nowrap text-left">
                            <div className="text-xs font-bold text-[var(--color-text-primary)]">{row} → {col}</div>
                            <div className="text-[10px] text-[var(--color-text-secondary)] mt-1">{cell.count} dependencies</div>
                            {Object.entries(cell.types).map(([type, count]) => (
                              <div key={type} className="text-[10px] text-[var(--color-text-muted)]">{type}: {count}</div>
                            ))}
