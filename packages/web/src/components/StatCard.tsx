...
export function StatCard({ label, value, icon, color, borderColor, suffix, trend }: StatCardProps) {
  ...
  return (
    <div
      className="relative overflow-hidden rounded-xl border bg-zinc-900/50 p-5 backdrop-blur-sm transition-all duration-300 hover:scale-[1.02] hover:shadow-lg hover:shadow-black/20"
      style={{ borderColor }}
    >
      {/* Glow effect */}
      <div
      ...
      {trend !== undefined && (
        <div className="mt-3 flex items-center gap-1 text-xs">
          ...
          <span className="text-zinc-600">vs last scan</span>
        </div>
      )}
    </div>
  );
