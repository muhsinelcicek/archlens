    ...
    <div className="p-6 lg:p-8 space-y-8 max-w-[1200px]">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--color-text-muted)] mt-1">{report.totalIssues} issues found across {report.modules.length} modules</p>
        </div>
        <div className="relative flex-shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="text-[9px] text-[var(--color-text-muted)]">QUALITY</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-1 border-b border-[var(--color-border-default)]">
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            className={`px-4 py-2.5 text-xs font-semibold transition-colors ${activeTab === tab.id ? "text-archlens-300 border-b-2 border-archlens-400" : "text-[var(--color-text-muted)] hover:text-[var(--color-text-secondary)]"}`}>
            {tab.label} {tab.count > 0 && <span className="ml-1 text-[9px] px-1.5 py-0.5 rounded-full bg-[var(--color-border-subtle)]">{tab.count}</span>}
          </button>
        ))}
      </div>
      ...
      <section>
        ...
        <div className="space-y-3">
          {(deepPatterns || report.architecturePatterns.map((p) => ({ ...p, id: p.pattern, status: p.compliance >= 80 ? "excellent" : p.compliance >= 50 ? "good" : "partial", summary: "", evidence: [], relatedPatterns: [] }))).map((pat: any) => {
            ...
            const statusColors: Record<string, string> = { excellent: "#34d399", good: "#60a5fa", partial: "#fbbf24", poor: "#ef4444", "not-detected": "var(--color-text-muted)" };
            const sc = statusColors[pat.status] || "var(--color-text-muted)";
            const evidence = pat.evidence || [];
            const violations = pat.violations || [];

            return (
              <div key={pat.id} className="rounded-xl border overflow-hidden" style={{ borderColor: isExpanded ? `${sc}40` : "var(--color-border-default)" }}>
                <button onClick={() => setExpandedPattern(isExpanded ? null : pat.id)} className="w-full flex items-center gap-3 px-5 py-4 hover:bg-hover transition-colors text-left">
                  ...
                  <span className="font-semibold text-[var(--color-text-primary)] flex-1">{pat.pattern}</span>
                ...
                {isExpanded && (
                  <div className="border-t border-[var(--color-border-default)]">
                    ...
                    <div className="px-5 py-3 bg-deep">
                      <p className="text-sm text-[var(--color-text-secondary)]">{pat.summary}</p>
                      {pat.relatedPatterns?.length > 0 && (
                        <div className="flex gap-1.5 mt-2">
                          {pat.relatedPatterns.map((rp: string) => <span key={rp} className="text-[9px] px-1.5 py-0.5 rounded bg-[var(--color-border-subtle)] text-[var(--color-text-muted)]">{rp}</span>)}
                        </div>
                      )}
                    </div>
                    ...
                    {evidence.length > 0 && (
                      <div className="px-5 py-3 border-t border-[var(--color-border-subtle)]">
                        <div className="text-[9px] uppercase font-semibold text-[var(--color-text-muted)] mb-2">Evidence ({evidence.length})</div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5">
                          {evidence.slice(0, 12).map((e: any, i: number) => {
                            const typeColors: Record<string, string> = { aggregate: "#fbbf24", "value-object": "#a78bfa", "domain-event": "#f472b6", entity: "#34d399", repository: "#60a5fa", interface: "#06b6d4", command: "#f97316", query: "#60a5fa", handler: "#34d399", service: "var(--color-text-secondary)", event: "#f472b6", controller: "#60a5fa" };
                            const tc = typeColors[e.type] || "var(--color-text-muted)";
                            return (
                              <div key={i} className="flex items-center gap-2 rounded-lg bg-[var(--color-border-subtle)] px-2.5 py-1.5">
                                <span className="text-[9px] px-1.5 py-0.5 rounded font-semibold" style={{ backgroundColor: `${tc}15`, color: tc }}>{e.type}</span>
                                <span className="text-[10px] font-mono text-[var(--color-text-primary)] truncate">{e.name}</span>
                                {e.details && <span className="text-[9px] text-[var(--color-text-muted)] ml-auto truncate max-w-[120px]">{e.details}</span>}
                              </div>
                            );
                          })}
                          {evidence.length > 12 && <div className="text-[9px] text-[var(--color-text-muted)] px-2">+{evidence.length - 12} more</div>}
                        </div>
                      </div>
                    )}
                    ...
                    {(pat.recommendations || []).length > 0 && (
                      <div className="px-5 py-3 border-t border-[var(--color-border-subtle)]">
                        <div className="text-[9px] uppercase font-semibold text-[var(--color-text-muted)] mb-2">Recommendations</div>
                        {pat.recommendations.map((r: string, i: number) => (
                          <div key={i} className="flex items-start gap-2 text-xs text-[var(--color-text-secondary)] py-0.5">
      ...
      <section>
        ...
        <div className="space-y-2">
          {report.modules.sort((a, b) => a.score - b.score).map((mod) => {
            ...
            return (
              <div key={mod.moduleName} className="rounded-xl border border-[var(--color-border-default)] overflow-hidden">
                <button onClick={() => setExpandedModule(isExpanded ? null : mod.moduleName)} className="w-full flex items-center gap-3 px-5 py-3 hover:bg-hover transition-colors text-left">
                  {isExpanded ? <ChevronDown className="h-4 w-4 text-[var(--color-text-muted)]" /> : <ChevronRight className="h-4 w-4 text-[var(--color-text-muted)]" />}
                  <span className="font-mono font-semibold text-sm text-[var(--color-text-primary)] flex-1">{mod.moduleName}/</span>
                  <span className="text-xs text-[var(--color-text-muted)]">{mod.issues.length} issues</span>
                    ...
                    <div className="px-5 py-3 bg-surface grid grid-cols-4 gap-3 text-center">
                      {[
                        ...
                        <div key={m.l} className="rounded-lg bg-elevated p-2">
                          <div className={`text-sm font-bold ${m.warn ? "text-amber-400" : "text-[var(--color-text-primary)]"}`}>{m.v}</div>
                          <div className="text-[8px] text-[var(--color-text-muted)] uppercase">{m.l}</div>
                        </div>
                      ))}
                    </div>
                    {/* Issues */}
                    <div className="px-5 py-2 max-h-[400px] overflow-y-auto divide-y divide-[var(--color-border-subtle)]">
                          ...
                          <div key={issue.id} className={`py-2 transition-all ${isSelected ? "bg-archlens-500/5 -mx-5 px-5 rounded-lg" : ""}`}>
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="text-xs text-[var(--color-text-primary)]">{issue.message}</div>
                                <div className="flex items-center gap-2 mt-1 text-[10px] text-[var(--color-text-muted)]">
                                  <span className="font-mono px-1 py-0.5 rounded bg-[var(--color-border-subtle)]">{issue.rule}</span>
                              ...
                              <ChevronRight className={`h-3 w-3 text-[var(--color-text-muted)] mt-1 transition-transform ${isSelected ? "rotate-90" : ""}`} />
                            </button>
                              ...
                              <div className="ml-7 mt-2 space-y-2 animate-slide-up">
                                ...
                                {issue.filePath && (
                                  <div className="flex items-center gap-2 text-[10px] font-mono text-[var(--color-text-secondary)]">
                                    <FileCode className="h-3 w-3 text-[var(--color-text-muted)]" />
                                ...
                                {issue.suggestion && (
                                  <div className="flex items-start gap-2 rounded-lg bg-amber-500/5 border border-amber-500/20 p-2.5">
                                    ...
                                    <div>
                                      ...
                                      <div className="text-xs text-[var(--color-text-secondary)]">{issue.suggestion}</div>
                                    </div>
                                  </div>
                                )}
                                ...
                                <div className="flex items-center gap-3 text-[10px]">
                                  ...
                                  <span className="text-[var(--color-text-muted)]">{issue.category}</span>
                                </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {[
              ...
              <div key={m.label} className={`rounded-xl border p-4 ${m.warn ? "border-amber-500/20 bg-amber-500/5" : "border-[var(--color-border-default)]"}`}>
                <div className="text-[10px] text-[var(--color-text-muted)] uppercase">{m.label}</div>
                <div className={`text-xl font-bold ${m.warn ? "text-amber-400" : "text-[var(--color-text-primary)]"}`}>{m.value}</div>
              </div>
            ))}
          </div>
            ...
            <section className="rounded-xl border border-amber-500/20 p-4">
              ...
              {coupling.circularDependencies.map((cd: any, i: number) => (
                <div key={i} className="flex items-center gap-2 py-1 text-xs">
                  ...
                  <span className="font-mono text-[var(--color-text-primary)]">{cd.cycle.join(" ↔ ")}</span>
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-[var(--color-border-subtle)] text-[var(--color-text-muted)]">{cd.level}</span>
                  <span className="text-[10px] text-[var(--color-text-muted)]">{cd.description}</span>
                </div>
              ))}
            </section>
          )}
          ...
          <section>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-[var(--color-border-default)] text-[var(--color-text-muted)] text-[10px] uppercase">
                ...
                <tbody>
                  {coupling.modules.slice(0, 15).map((m: any) => {
                    ...
                    return (
                      <tr key={m.moduleName} className="border-b border-[var(--color-border-subtle)] hover:bg-hover">
                        <td className="py-2 px-3 font-mono text-[var(--color-text-primary)]">{m.moduleName}</td>
                        <td className="text-right px-2 text-[var(--color-text-secondary)]">{m.afferentCoupling}</td>
                        <td className="text-right px-2 text-[var(--color-text-secondary)]">{m.efferentCoupling}</td>
                        <td className="text-right px-2 font-bold" style={{ color: iColor }}>{m.instability}</td>
                        <td className="text-right px-2 text-[var(--color-text-secondary)]">{m.abstractness}</td>
                        <td className="text-right px-2 text-[var(--color-text-secondary)]">{m.distanceFromMainSeq}</td>
      ...
      {activeTab === "consistency" && consistency && (
        <div className="space-y-6">
          <p className="text-sm text-[var(--color-text-secondary)]">{consistency.summary}</p>
          ...
          <section>
            ...
            <div className="space-y-2">
              {consistency.moduleScores.sort((a: any, b: any) => a.overall - b.overall).map((m: any) => (
                <div key={m.module} className="flex items-center gap-3 rounded-lg border border-[var(--color-border-default)] px-4 py-2">
                  <span className="font-mono text-sm text-[var(--color-text-primary)] w-40 truncate">{m.module}</span>
                  <div className="flex-1 flex items-center gap-4">
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] text-[var(--color-text-muted)]">Error Handling</span>
                      <div className="w-20 h-1.5 rounded-full bg-[var(--color-border-subtle)]"><div className="h-full rounded-full" style={{ width: `${m.errorHandling}%`, backgroundColor: m.errorHandling >= 80 ? "#34d399" : m.errorHandling >= 50 ? "#fbbf24" : "#ef4444" }} /></div>
                      <span className="text-[10px] font-bold" style={{ color: m.errorHandling >= 80 ? "#34d399" : "#fbbf24" }}>{m.errorHandling}%</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] text-[var(--color-text-muted)]">Logging</span>
                      <div className="w-20 h-1.5 rounded-full bg-[var(--color-border-subtle)]"><div className="h-full rounded-full" style={{ width: `${m.logging}%`, backgroundColor: m.logging >= 80 ? "#34d399" : m.logging >= 50 ? "#fbbf24" : "#ef4444" }} /></div>
            ...
            <section>
              ...
              <div className="space-y-2">
                {consistency.issues.map((issue: any, i: number) => (
                  <div key={i} className="rounded-lg border border-[var(--color-border-default)] px-4 py-3">
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      <span className="text-[9px] px-1.5 py-0.5 rounded bg-[var(--color-border-subtle)] text-[var(--color-text-muted)]">{issue.category}</span>
                      <span className="font-mono text-[var(--color-text-secondary)]">{issue.module}</span>
                    </div>
                    <div className="text-xs text-[var(--color-text-primary)] mt-1">{issue.description}</div>
                    <div className="text-[10px] text-[var(--color-text-muted)] mt-0.5">{issue.evidence}</div>
                  </div>
                ))}
              </div>
            </section>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-3 gap-4">
            <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-5">
              <div className="text-xs text-[var(--color-text-muted)] uppercase mb-2">{t("debt.total_fix")}</div>
              <div className="text-3xl font-bold text-red-400">${(techDebt.totalEstimatedCost / 1000).toFixed(0)}k</div>
              <div className="text-xs text-[var(--color-text-muted)] mt-1">{techDebt.totalEstimatedHours} {t("debt.hours")}</div>
            </div>
            <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-5">
              <div className="text-xs text-[var(--color-text-muted)] uppercase mb-2">{t("debt.annual")}</div>
              <div className="text-3xl font-bold text-amber-400">${(techDebt.totalAnnualCost / 1000).toFixed(0)}k</div>
              <div className="text-xs text-[var(--color-text-muted)] mt-1">{t("debt.ongoing")}</div>
            </div>
            <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-5">
              <div className="text-xs text-[var(--color-text-muted)] uppercase mb-2">{t("debt.best_roi")}</div>
              <div className="text-3xl font-bold text-emerald-400">{techDebt.quickWins.length}</div>
              <div className="text-xs text-[var(--color-text-muted)] mt-1">{t("debt.quick_wins")}</div>
            </div>
          </div>
          {techDebt.items.map((item: any, i: number) => (
            <div key={i} className="rounded-xl border border-[var(--color-border-default)] p-4 flex items-center justify-between">
              <div>
                <div className="font-semibold text-[var(--color-text-primary)]">{item.category}</div>
                <div className="text-xs text-[var(--color-text-secondary)] mt-0.5">{item.description}</div>
              </div>
              <div className="flex items-center gap-4 text-sm">
                <span className="text-[var(--color-text-primary)]">${(item.estimatedCost / 1000).toFixed(1)}k</span>
