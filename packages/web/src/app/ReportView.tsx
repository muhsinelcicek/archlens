  ...
  return (
    <>
      ...
      <div className="report-page p-6 lg:p-8 max-w-[900px] mx-auto space-y-6">
        ...
        <div className="no-print flex items-center justify-between rounded-xl border border-[var(--color-border-default)] bg-elevated p-4">
          <div className="flex items-center gap-3">
            <FileText className="h-5 w-5 text-archlens-400" />
            <div>
              <h2 className="text-base font-bold">Executive Report</h2>
              <p className="text-xs text-[var(--color-text-muted)]">Print or export this page as PDF for stakeholders</p>
            </div>
          </div>
          <div className="flex gap-2">
            <button
              onClick={downloadJson}
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[var(--color-border-subtle)] border border-[var(--color-border-default)] text-[var(--color-text-secondary)] text-xs font-semibold hover:text-[var(--color-text-primary)]"
            >
        ...
        {/* Cover */}
        <section className="print-section rounded-xl border border-[var(--color-border-default)] p-8 text-center">
          <div className="text-[10px] uppercase font-semibold text-[var(--color-text-muted)] tracking-widest mb-3">ArchLens Architecture Report</div>
          <h1 className="text-4xl font-bold text-[var(--color-text-primary)] mb-2">{model.project.name}</h1>
          <p className="text-sm text-[var(--color-text-secondary)]">{model.project.rootPath}</p>
          <p className="text-xs text-[var(--color-text-muted)] mt-1">Analyzed: {new Date(model.project.analyzedAt).toLocaleString()}</p>
          ...
          <div className="mt-6 flex justify-center">
            <div className="relative w-32 h-32">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center keep-color">
                ...
                <span className="text-[9px] text-[var(--color-text-muted)] uppercase">Quality Score</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="print-section">
          <h2 className="text-lg font-bold mb-3 border-b border-[var(--color-border-default)] pb-2">1. Project Overview</h2>
          <div className="grid grid-cols-4 gap-3 mb-4">
            {[
              ...
              <div key={s.l} className="rounded-lg border border-[var(--color-border-default)] p-3 text-center">
                <div className="text-xl font-bold text-[var(--color-text-primary)]">{s.v}</div>
                <div className="text-[9px] uppercase text-[var(--color-text-muted)]">{s.l}</div>
              </div>
            ))}
          </div>

          <div className="text-xs">
            <div className="font-semibold text-[var(--color-text-secondary)] mb-1">Languages</div>
            <div className="flex flex-wrap gap-2">
              {langs.map(([lang, count]) => (
                <span key={lang} className="px-2 py-1 rounded bg-[var(--color-border-subtle)] text-[var(--color-text-primary)]">
                  {lang}: <span className="text-[var(--color-text-muted)]">{count as number}</span>
                </span>
              ))}
            </div>
          </div>
        ...
        {quality && (
          <section className="print-section">
            <h2 className="text-lg font-bold mb-3 border-b border-[var(--color-border-default)] pb-2">2. Code Quality</h2>
        ...
        {coupling && (
          <section className="print-section">
            <h2 className="text-lg font-bold mb-3 border-b border-[var(--color-border-default)] pb-2">3. Coupling &amp; Dependencies</h2>
            <p className="text-sm text-[var(--color-text-secondary)] mb-3">
            ...
            {coupling.circularDependencies.length > 0 && (
              <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3">
                ...
                {coupling.circularDependencies.slice(0, 5).map((cd: any, i: number) => (
                  <div key={i} className="text-xs font-mono text-[var(--color-text-secondary)]">⟳ {(cd.cycle || []).join(" ↔ ")}</div>
                ))}
              </div>
            )}
          </section>
        ...
        {security && (
          <section className="print-section">
            <h2 className="text-lg font-bold mb-3 border-b border-[var(--color-border-default)] pb-2">4. Security</h2>
        ...
        {deadcode && (
          <section className="print-section">
            <h2 className="text-lg font-bold mb-3 border-b border-[var(--color-border-default)] pb-2">5. Dead Code</h2>
            <p className="text-sm text-[var(--color-text-secondary)]">
        ...
        {techDebt && (
          <section className="print-section">
            <h2 className="text-lg font-bold mb-3 border-b border-[var(--color-border-default)] pb-2">6. Technical Debt</h2>
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-lg border border-[var(--color-border-default)] p-3">
                <div className="text-[9px] uppercase text-[var(--color-text-muted)]">Total Fix Cost</div>
              ...
              <div className="rounded-lg border border-[var(--color-border-default)] p-3">
                <div className="text-[9px] uppercase text-[var(--color-text-muted)]">Annual Cost (ongoing)</div>
                <div className="text-xl font-bold text-amber-400 keep-color">${(techDebt.totalAnnualCost / 1000).toFixed(0)}k</div>
                <div className="text-[10px] text-[var(--color-text-muted)]">if not addressed</div>
              </div>
              <div className="rounded-lg border border-[var(--color-border-default)] p-3">
                <div className="text-[9px] uppercase text-[var(--color-text-muted)]">Quality Score</div>
                <div className="text-xl font-bold keep-color" style={{ color: scoreColor(overallScore) }}>{overallScore}/100</div>
                <div className="text-[10px] text-[var(--color-text-muted)]">overall</div>
              </div>
            </div>
          </section>
        )}
        ...
        {/* Module Breakdown */}
        <section className="print-section">
          <h2 className="text-lg font-bold mb-3 border-b border-[var(--color-border-default)] pb-2">7. Module Breakdown</h2>
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-[var(--color-border-default)] text-left text-[10px] uppercase text-[var(--color-text-muted)]">
            ...
            <tbody>
              {[...model.modules].sort((a, b) => b.lineCount - a.lineCount).slice(0, 20).map((m) => (
                <tr key={m.name} className="border-b border-[var(--color-border-subtle)]">
                  <td className="py-1.5 font-mono text-[var(--color-text-primary)]">{m.name}</td>
                  <td className="text-[var(--color-text-secondary)]">{m.layer}</td>
                  <td className="text-[var(--color-text-secondary)]">{m.language}</td>
                  <td className="text-right text-[var(--color-text-secondary)]">{m.fileCount}</td>
                  <td className="text-right text-[var(--color-text-secondary)]">{m.lineCount.toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {/* Footer */}
        <div className="text-center text-[10px] text-[var(--color-text-muted)] pt-4 border-t border-[var(--color-border-default)]">
