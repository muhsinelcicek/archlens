  ...
  return (
    <div className="p-6 lg:p-8 space-y-10 max-w-[1100px]">
        ...
        <div className="relative">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-[#5a5a70] text-sm">Onboarding guide — everything you need to understand this project</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
            {[
              ...
              <div key={stat.label} className="rounded-lg bg-elevated border border-[#2a2a3a] p-3 text-center">
                <stat.icon className="h-4 w-4 text-archlens-500 mx-auto mb-1" />
                <div className="text-xl font-bold text-[#e4e4ed]">{stat.value}</div>
                <div className="text-[10px] text-[#5a5a70] uppercase">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-[#5a5a70] mb-3">{meta.description}</p>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {mods.map((mod) => (
                          ...
                          >
                            ...
                            <div className="flex gap-3 text-[10px] text-[#5a5a70] mt-1">
      ...
      <section>
        ...
        <div className="rounded-xl border border-[#2a2a3a] bg-surface p-5">
            ...
            {[...depMap.entries()].map(([src, deps]) => {
              ...
              return (
                <div key={src} className="flex items-center gap-3 flex-wrap">
                  ...
                  <ArrowRight className="h-4 w-4 text-[#5a5a70]" />
      ...
      <section>
        ...
        <div className="space-y-3">
          {processes.filter((p) => p.category !== "presentation" && p.category !== "api-service").map((proc) => {
            ...
            return (
              <div key={proc.id} className="rounded-xl border bg-surface p-5" style={{ borderColor: `${color}20` }}>
                <div className="flex items-center gap-3 mb-3">
                  <div className="h-2 w-2 rounded-full" style={{ backgroundColor: color }} />
                  <h3 className="font-semibold text-[#e4e4ed]">{proc.name}</h3>
                  <span className="text-[10px] rounded-full px-2 py-0.5" style={{ backgroundColor: `${color}15`, color }}>
                    {proc.category}
                  </span>
                </div>
                <p className="text-sm text-[#8888a0] mb-4">{proc.description}</p>
                ...
                <div className="flex items-center gap-1 overflow-x-auto pb-2">
                  ...
                  <ArrowRight className="h-3.5 w-3.5 text-[#5a5a70] flex-shrink-0" />
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {model.dbEntities.map((entity) => (
              <div key={entity.name} className="rounded-xl border border-[#2a2a3a] bg-surface p-4">
                ...
                <div className="space-y-0.5">
                  {entity.columns.slice(0, 6).map((col) => (
                    <div key={col.name} className="flex items-center gap-2 text-[11px] font-mono">
                      ...
                      <span className="text-[#8888a0]">{col.name}</span>
                      <span className="text-[#5a5a70] ml-auto">{col.type}</span>
                    </div>
                  ))}
                  {entity.columns.length > 6 && (
                    <div className="text-[10px] text-[#5a5a70]">+{entity.columns.length - 6} more columns</div>
                  )}
                </div>
              </div>
            ))}
          ...
          {(() => {
            ...
            return [...byCat.entries()].map(([cat, items]) => (
              <div key={cat} className="mb-4 last:mb-0">
                <h4 className="text-xs font-semibold uppercase tracking-wider text-[#5a5a70] mb-2">{cat}</h4>
                <div className="flex flex-wrap gap-2">
                  {items.map((t) => (
                    <span key={t.name} className="inline-flex items-center gap-1.5 rounded-md bg-hover border border-zinc-700/50 px-2.5 py-1 text-xs">
                      <span className="text-[#8888a0]">{t.name}</span>
                      {t.version && <span className="text-[#5a5a70]">{t.version}</span>}
                    </span>
                  ))}
                </div>
              </div>
      ...
      {/* Footer tip */}
      <div className="rounded-xl border border-[#2a2a3a] bg-surface p-4 text-center text-sm text-[#5a5a70]">
