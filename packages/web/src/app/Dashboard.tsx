  ...
  return (
    <div className="p-6 lg:p-8 space-y-8 max-w-[1400px]">
      ...
      <section className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-6 backdrop-blur-sm">
        <h3 className="text-sm font-semibold uppercase tracking-wider text-zinc-400 mb-4">Languages</h3>
      ...
      <section className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-6 backdrop-blur-sm">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h3 className="text-sm font-semibold uppercase tracking-wider text-zinc-400">Architecture Layers</h3>
        </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-medium capitalize text-zinc-200">{layer}</span>
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-1.5">
                      {mods.map((mod) => (
                        ...
                        >
                          {mod.name}/
                          <span className="ml-1 text-zinc-500">{mod.fileCount}f</span>
                        </span>
                      ))}
                    </div>
                  </div>
      ...
      {/* Modules Detail Grid */}
      <section className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-6 backdrop-blur-sm">
        <h3 className="text-sm font-semibold uppercase tracking-wider text-zinc-400 mb-4">Modules</h3>
              ...
              >
                ...
                <div className="relative">
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-4 text-center">
                    <div>
                      <div className="text-xl font-bold text-zinc-200">{mod.fileCount}</div>
                      <div className="text-[10px] text-zinc-600 uppercase">files</div>
                    </div>
                    <div>
                      <div className="text-xl font-bold text-zinc-200">{mod.symbols.length}</div>
                      <div className="text-[10px] text-zinc-600 uppercase">symbols</div>
                    </div>
                    <div>
                      <div className="text-xl font-bold text-zinc-200">{mod.lineCount.toLocaleString()}</div>
                      <div className="text-[10px] text-zinc-600 uppercase">lines</div>
                    </div>
                  </div>
                  <div className="mt-2 text-xs text-zinc-600 font-mono">{mod.language}</div>
                </div>
              </div>
            );
          })}
      ...
      {/* Tech Stack */}
      <section className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-6 backdrop-blur-sm">
        <h3 className="text-sm font-semibold uppercase tracking-wider text-zinc-400 mb-4">Tech Stack</h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
          {techRadar.slice(0, 24).map((tech) => (
            <div
              key={tech.name}
              className="flex items-center gap-2.5 rounded-lg border border-zinc-800/50 bg-zinc-800/30 px-3 py-2.5 text-sm transition-colors hover:border-zinc-700 hover:bg-zinc-800/60"
            >
              <div className="h-2 w-2 rounded-full bg-archlens-500" />
              <span className="text-zinc-300 truncate">{tech.name}</span>
              {tech.version && (
                <span className="ml-auto text-[10px] text-zinc-600 font-mono">{tech.version}</span>
              )}
            </div>
          ))}
        </div>
