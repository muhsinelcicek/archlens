...
const layerBadge: Record<string, string> = {
  ...
  unknown: "bg-zinc-500/20 text-[#8888a0]",
};
        ...
        {sorted.map((mod) => (
          ...
          >
            <div className="flex items-start justify-between mb-3">
              ...
              <span className="text-xs text-[#5a5a70] font-mono">{mod.language}</span>
            </div>

            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                <div className="text-xl font-bold">{mod.fileCount}</div>
                <div className="text-xs text-[#5a5a70]">files</div>
              </div>
              <div>
                <div className="text-xl font-bold">{mod.symbols.length}</div>
                <div className="text-xs text-[#5a5a70]">symbols</div>
              </div>
              <div>
                <div className="text-xl font-bold">{mod.lineCount.toLocaleString()}</div>
                <div className="text-xs text-[#5a5a70]">lines</div>
              </div>
            </div>
          </div>
        ))}
